...
// Toast
const toast = document.getElementById('toast') || (() => { const el = document.createElement('div'); el.id = 'toast'; el.className = 'toast'; document.body.appendChild(el); return el; })();
let toastTimer;
function showToast(msg, type) {
    if (toastTimer) clearTimeout(toastTimer);
    toast.textContent = msg; toast.className = 'toast ' + (type === 'success' ? 'success' : '');
...
// 灯箱
const lightbox = document.getElementById('lightboxOverlay'), lightboxImg = document.getElementById('lightboxImage'),
...
function showImage(i) {
    ...
    curIdx = i; lightboxImg.src = gallery[i];
    counter.textContent = `${curIdx + 1} / ${gallery.length}`;
    prevBtn.style.display = curIdx > 0 ? 'flex' : 'none';
    nextBtn.style.display = curIdx < gallery.length - 1 ? 'flex' : 'none';
}
function closeLightbox() { lightbox.classList.remove('active'); setTimeout(() => { lightboxImg.src = ''; gallery = []; }, 300); }
prevBtn.addEventListener('click', (e) => { e.stopPropagation(); if (curIdx > 0) showImage(curIdx - 1); });
nextBtn.addEventListener('click', (e) => { e.stopPropagation(); if (curIdx < gallery.length - 1) showImage(curIdx + 1); });
...
// 弹窗
const shareModal = document.getElementById('shareModal'), tutorialModal = document.getElementById('tutorialModal');
if (shareModal && tutorialModal) {
    ...
    [shareModal, tutorialModal].forEach(m => m.addEventListener('click', (e) => { if (e.target === m) m.classList.remove('active'); }));
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { shareModal.classList.remove('active'); tutorialModal.classList.remove('active'); } });
}

// 消息气泡
document.getElementById('msgBubble')?.addEventListener('click', () => {
    try {
