...
function showDataFallbackModal() {
    ...
    if (contentDiv) {
        let html = '<p>数据加载失败，请选择备用线路：</p>';
        DATA_SOURCES.forEach((url, i) => {
            html += `<button class="line-btn" style="margin:8px auto; display:block;" data-url="${url}">🚀 线路${i+1}：${url.split('/').pop()}</button>`;
        });
